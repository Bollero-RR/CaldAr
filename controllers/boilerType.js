...
const idFilter = (req) => (boilerType) =>
  boilerType.id === parseInt(req.params.id);
...
router.get("/skillsId/:skillsId", (req, res) => {
  const found = boilerType.some(
    (boilerType) => boilerType.skillsId === req.params.skillsId
  );
  ...
  if (found) {
    ...
    res
      .status(400)
      .send({
        msg: `Boiler Type not found with this skill Id: ${req.params.skillsId}`,
      });
  }
});
