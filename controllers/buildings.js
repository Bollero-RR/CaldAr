...
router.get("/:id", (req, res) => {
  ...
  if (found) {
    ...
    res
      .status(400)
      .json({ msg: `Buildind not found with the id: ${req.params.id}` });
  }
});
...
router.get("/fullName/:fullName", (req, res) => {
  const found = getBuildings.some(
    (getBuildings) => getBuildings.fullName === req.params.fullName
  );
  if (found) {
    res.json(
      getBuildings.filter(
...
router.delete("/:id", (req, res) => {
  ...
  if (found) {
    res.json({
      ...
      members: getBuildings.filter(
        (getBuildings) => !idFilter(req)(getBuildings)
      ),
    });
  } else {
    res
